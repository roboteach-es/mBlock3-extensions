...
(function(ext) {
	...
	*******************************/
	...
	function deviceOpened(dev) {
		...
		device.set_receive_handler('Input-PullUP',function(data) {
			processData(data);
		});
	};
	...
	ext._getStatus = function() {
		if(!device) return {status: 1, msg: 'Input-PullUP disconnected'};
		if(watchdog) return {status: 1, msg: 'Probing for Input-PullUP'};
		return {status: 2, msg: 'Input-PullUP connected'};
	}

	var descriptor = {};
	ScratchExtensions.register('Input-PullUP', descriptor, ext, {type: 'serial'});
})({});
